...
export async function getStaticPaths() {
  const projectsRecords = await base.table('Projects').select().all()
  const projects = projectsRecords.map(project => minifyRecord(project))
...
export async function getStaticProps({ params }) {
  const record = await base.table('Projects').find(params.id)
